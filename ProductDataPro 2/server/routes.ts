...
import { enhanceProduct, enhanceProducts, getEnhancementSummary } from './services/enhancement-service';
import { registerAgentRoutes } from './services/fixed-agent-api';
import { generateSEOKeywords, analyzeKeywordUsage } from './services/seo-keyword-service';
import { generateImageAltText } from './services/image-alt-text-service';
import { checkPolicyCompliance } from './services/policy-compliance-service';
import { validateProductFields } from './services/field-validation-service';
...
export async function registerRoutes(app: Express): Promise<Server> {
  // Register MarketMind Agent routes
  ...
  // Enhanced product data with comprehensive analysis and optimization
  app.post("/api/enhance/comprehensive", async (req, res) => {
    try {
      const { products, productIds, marketplace } = req.body;
      let productsToEnhance = [];
      
      console.log("Comprehensive enhance endpoint called with:", {
        hasProducts: products && Array.isArray(products), 
        productsLength: products?.length || 0,
        hasProductIds: productIds && Array.isArray(productIds),
        productIdsLength: productIds?.length || 0,
      ...
      // Call our comprehensive enhancement service
      console.log(`Starting comprehensive enhancement for ${productsToEnhance.length} products targeting ${marketplace}`);
      const enhancementResults = await enhanceProducts(productsToEnhance, marketplace);
      console.log(`Comprehensive enhancement completed for ${enhancementResults.length} products`);
      
      // Get summary of results
      const summary = getEnhancementSummary(enhancementResults);
      
      // Extract enhanced products and update in database
      const enhancedProducts = enhancementResults.map(result => result.product);
      let updatedProducts = [];
      
      try {
        // First try to update existing products
        updatedProducts = await storage.updateProducts(enhancedProducts);
        console.log(`Updated ${updatedProducts.length} existing products`);
      } catch (updateError) {
        console.warn("Failed to update products, attempting to save as new:", updateError);
        // If update fails, save as new products
        try {
          updatedProducts = await storage.saveProducts(enhancedProducts);
          console.log(`Saved ${updatedProducts.length} new products`);
        } catch (saveError) {
          console.error("Failed to save products:", saveError);
          // Continue anyway to return the enhancement results to the client
        }
      }
      
      // Return all results even if database operations failed
      res.json({
        message: "Comprehensive product enhancement completed",
        products: enhancedProducts, // Return the original enhanced products
        summary: summary,
        enhancementResults: enhancementResults,
        savedToDatabase: updatedProducts.length > 0
      });
  ...
  // SEO keyword generation endpoint
  app.post("/api/seo/keywords", async (req, res) => {
    try {
      ...
      console.log(`Generating SEO keywords for ${marketplace}`);
      const keywords = await generateSEOKeywords(product, marketplace);
      
      // If we have product text content, analyze keyword usage
      let analysis = null;
      if (product.description) {
        console.log("Analyzing keyword usage in product description");
        analysis = analyzeKeywordUsage(product.description, keywords);
      }
      ...
      const errorMessage = error instanceof Error ? error.message : "Unknown error";
      res.status(500).json({ 
        message: "Error generating SEO keywords", 
        error: errorMessage 
      });
    }
  });
  
  // Image alt text generation endpoint
  app.post("/api/images/alt-text", async (req, res) => {
    try {
      const { imageContext, marketplace, maxLength } = req.body;
      
      if (!imageContext || !marketplace) {
        return res.status(400).json({ 
          message: "Missing required parameters",
          error: "Both imageContext and marketplace parameters are required" 
        });
      }
      
      // Generate image alt text
      console.log(`Generating image alt text for ${marketplace}`);
      const altText = await generateImageAltText(imageContext, marketplace, maxLength || 125);
      ...
      const errorMessage = error instanceof Error ? error.message : "Unknown error";
      res.status(500).json({ 
        message: "Error generating image alt text", 
        error: errorMessage 
      });
    }
  });
  
  // Policy compliance check endpoint
  app.post("/api/compliance/check", async (req, res) => {
    try {
      ...
      console.log(`Checking policy compliance for ${marketplace}`);
      ...
      const errorMessage = error instanceof Error ? error.message : "Unknown error";
      res.status(500).json({ 
        message: "Error checking policy compliance", 
        error: errorMessage 
      });
    }
  });
  
  // Direct product enhancement endpoint (no database saving)
  app.post("/api/direct_enhance", async (req, res) => {
    try {
      const { product, marketplace } = req.body;
      ...
      // Call the enhancement service directly for a single product
      console.log(`Starting direct enhancement for marketplace: ${marketplace}`);
      const enhancementResult = await enhanceProduct(product, marketplace);
      
      res.json({
        message: "Product enhancement completed",
        enhancedProduct: enhancementResult.product,
        seoKeywords: enhancementResult.seoKeywords,
        imageAltTexts: enhancementResult.imageAltTexts,
        complianceResult: enhancementResult.complianceResult,
        validationResult: enhancementResult.validationResult,
        issues: enhancementResult.issues,
        score: enhancementResult.score
      });
      
    } catch (error) {
      console.error("Error in direct enhancement:", error);
      const errorMessage = error instanceof Error ? error.message : "Unknown error";
      res.status(500).json({ 
        message: "Error enhancing product", 
        error: errorMessage 
      });
    }
  });
  ...
  app.post("/api/validation/fields", async (req, res) => {
    try {
      const { product, marketplace } = req.body;
      ...
      console.log(`Validating product fields for ${marketplace}`);
      ...
      console.error("Error validating product fields:", error);
      const errorMessage = error instanceof Error ? error.message : "Unknown error";
      res.status(500).json({ 
        message: "Error validating product fields", 
        error: errorMessage 
      });
    }
  });
  
  // REST API endpoint to get enhanced product by ID
  app.get("/api/enhanced_products/:productId", async (req, res) => {
    try {
      ...
      console.log(`Getting enhanced product with ID: ${productId}`);
      ...
      res.json({
        message: "Enhanced product retrieved successfully",
        product
      });
      
    } catch (error) {
      console.error("Error retrieving enhanced product:", error);
      const errorMessage = error instanceof Error ? error.message : "Unknown error";
      res.status(500).json({ 
        message: "Error retrieving enhanced product", 
        error: errorMessage 
      });
    }
  });
  
  // REST API endpoint to get all enhanced products
  app.get("/api/enhanced_products", async (req, res) => {
    try {
      // Get all products from database
      console.log("Getting all enhanced products");
      const products = await storage.getProducts();
      
      // Filter for only enhanced products if specified in query
      const { enhanced_only } = req.query;
      let filteredProducts = products;
      
      if (enhanced_only === 'true') {
        filteredProducts = products.filter(p => 
          p.status === 'enhanced' || p.status === 'enhanced_with_issues'
        );
      }
      ...
      console.error("Error retrieving enhanced products:", error);
      const errorMessage = error instanceof Error ? error.message : "Unknown error";
      res.status(500).json({ 
        message: "Error retrieving enhanced products", 
        error: errorMessage 
      });
    }
  });
